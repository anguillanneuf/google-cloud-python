...
def parse_options():
    ...
    parser.add_argument(
        ...
        dest="workload",
        default="",
        help="The path to a YCSB workload file.",
    )
    parser.add_argument(
        "-p",
        "--parameter",
        ...
        dest="parameters",
        default=[],
        help="The key=value pair of parameter.",
    )
    parser.add_argument(
        "-b",
        "--num_bucket",
...
def load_keys(database, parameters):
    ...
    with database.snapshot() as snapshot:
        results = snapshot.execute_sql("SELECT u.id FROM %s u" % parameters["table"])
...
def read(database, table, key):
    ...
    with database.snapshot() as snapshot:
        result = snapshot.execute_sql(
            'SELECT u.* FROM %s u WHERE u.id="%s"' % (table, key)
        )
        for row in result:
            key = row[0]
...
def update(database, table, key):
    ...
    with database.batch() as batch:
        batch.update(
            table=table, columns=("id", "field%d" % field), values=[(key, value)]
        )
...
def aggregate_metrics(latencies_ms, duration_ms, num_bucket):
    ...
    overall_op_count = 0
    op_counts = {
        operation: len(latency) for operation, latency in latencies_ms.iteritems()
    }
    overall_op_count = sum([op_count for op_count in op_counts.itervalues()])

    print("[OVERALL], RunTime(ms), %f" % duration_ms)
    print(
        "[OVERALL], Throughput(ops/sec), %f"
        % (float(overall_op_count) / duration_ms * 1000.0)
    )
    ...
    for operation in op_counts.keys():
        ...
        print(
            "[%s], 99.9thPercentileLatency(us), %f"
            % (
                operation_upper,
...
class WorkloadThread(threading.Thread):
    ...
    def __init__(self, database, keys, parameters, total_weight, weights, operations):
...
def run_workload(database, keys, parameters):
    ...
    for i in range(int(parameters["num_worker"])):
        thread = WorkloadThread(
            database, keys, parameters, total_weight, weights, operations
        )
        thread.start()
...
if __name__ == "__main__":
    ...
    if parameters["command"] == "run":
        if "cloudspanner.channels" in parameters:
            assert (
                parameters["cloudspanner.channels"] == 1
